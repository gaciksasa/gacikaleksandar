...
// If tag name is not provided, fetch the first existing tag name for the selected language
if (empty($tag_name)) {
  $sql = "SELECT name FROM tags WHERE language = ? LIMIT 1";
  ...
  if (empty($tag_name)) {
    die("No tags found for the selected language.");
  }
}

// Fetch tag details
$tag_name_clean = $conn->real_escape_string($tag_name);
$sql = "SELECT id FROM tags WHERE name = '$tag_name_clean' AND language = '$lang'";
...
$sql = "SELECT blog_posts.id, blog_posts.title, blog_posts.content, blog_posts.author, blog_posts.featured_image, blog_posts.published_date, blog_posts.slug
        FROM blog_posts
        JOIN blog_post_tags ON blog_posts.id = blog_post_tags.blog_post_id
        WHERE blog_post_tags.tag_id = ? AND blog_posts.language = ?";
$stmt = $conn->prepare($sql);
if ($stmt === false) {
  die("Prepare failed: " . $conn->error);
}
$stmt->bind_param("is", $tag_id, $lang);
$stmt->execute();
...
<head>
  ...
  <title><?php echo $translations['tag']; ?>: <?php echo htmlspecialchars($tag_name); ?></title>
          ...
          <div class="pbmit-title-bar-content-inner">
            <div class="pbmit-tbar">
              <div class="pbmit-tbar-inner container">
                <h1 class="pbmit-tbar-title"><?php echo $translations['tag']; ?>: <?php echo htmlspecialchars($tag_name); ?></h1>
              </div>
            </div>
          </div>
        </div>
